...
export const WalletSelectButton = (props) => {
  ...
  return (
    <>
      ...
      {showModal && (
        <WalletModal onClose={() => setShowModal(false)} onConnected={(walletName) => onWalletSelected(walletName)} />
      )}
    </>
  )
}
  ...
  async function connectWallet(walletName) {
    switch (walletName) {
      ...
      case ARWEAVE_APP:
        ...
        webWallet.on('change', () => {
          if (!webWallet.address && props.onDisconnected) props.onDisconnected()
        })
        break
      default:
